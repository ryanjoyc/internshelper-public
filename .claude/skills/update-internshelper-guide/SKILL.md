...
The guide lives at `.claude/skills/internshelper-guide/SKILL.md`.
...
- Added, removed, or renamed a **skill** in `.claude/skills/`.
...
1. **Read** `.claude/skills/internshelper-guide/SKILL.md`.
...
5. **Fix cross-references** if a name changed — the `add-source` / `review-internships` /
   `update-internshelper-guide` pointers, and the root `CLAUDE.md` if a skill name changed.

...
def test_select_for_digest_respects_watermark_filter_and_order(tmp_path):
    c = _conn(tmp_path)
    # New + matching (cs+intern) at different times
    store.upsert(c, _p("greenhouse:new1"), now="2026-06-18T10:00:00+00:00")
    store.upsert(c, _p("greenhouse:new2"), now="2026-06-18T11:00:00+00:00")
    # Old (before watermark) matching
    store.upsert(c, _p("greenhouse:old"), now="2026-06-18T08:00:00+00:00")
    # New but non-matching (cs only)
    store.upsert(c, _p("greenhouse:csonly", intern=False, newgrad=False),
                 now="2026-06-18T10:30:00+00:00")
    # New, matching, but closed
    store.upsert(c, _p("greenhouse:closed"), now="2026-06-18T10:45:00+00:00")
    c.execute("UPDATE postings SET is_active=0 WHERE posting_id='greenhouse:closed'")
    c.commit()

    rows = store.select_for_digest(
        c, last_digest_at="2026-06-18T09:00:00+00:00",
        require_cs=True, require_intern_or_newgrad=True,
    )
    ids = [r["posting_id"] for r in rows]
    assert ids == ["greenhouse:new2", "greenhouse:new1"]  # newest first; old/csonly/closed excluded
...
def test_record_run_and_read_back(tmp_path):
    ...
    store.record_run(c, source_key="greenhouse:stripe", ok=True, count=3,
                     error=None, now="2026-06-18T10:00:00+00:00")
    store.record_run(c, source_key="lever:netflix", ok=False, count=0,
                     error="boom", now="2026-06-18T10:00:01+00:00")
    rows = c.execute("SELECT source_key, ok, count, error FROM runs ORDER BY id").fetchall()

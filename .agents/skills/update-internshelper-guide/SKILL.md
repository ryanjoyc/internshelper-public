...
The guide lives at `.Codex/skills/internshelper-guide/SKILL.md`.
...
- Added, removed, or renamed a **skill** in `.Codex/skills/`.
...
1. **Read** `.Codex/skills/internshelper-guide/SKILL.md`.
...
5. **Fix cross-references** if a name changed — the `add-source` / `review-internships` /
   `update-internshelper-guide` pointers, and the root `AGENTS.md` if a skill name changed.
